...
const CLAUDE_PRICING_PER_1M: Record<string, ModelPricing> = {
  // ── Claude 5 family (Anthropic list prices, cache read at 10% of input) ─
  "claude-sonnet-5": { input: 2.0, input_cached: 0.2, output: 10.0 },
  "claude-opus-5": { input: 5.0, input_cached: 0.5, output: 25.0 },
  "claude-haiku-4-5": { input: 1.0, input_cached: 0.1, output: 5.0 },
...
export function parseClaudeJsonl(
  ...
  for (const line of lines) {
    ...
    if (entry.type === "assistant") {
      ...
      // Claude stream-json nests tool_use blocks inside assistant content.
