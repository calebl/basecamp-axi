...
import type { AgentBackend, GradingSpec, GradeResult } from "./types.ts";

const CODEX_JUDGE_MODEL = "gpt-5.4-mini";
const CLAUDE_JUDGE_MODEL = process.env.BENCH_JUDGE_MODEL ?? "claude-sonnet-5";
...
export function grade(
  ...
  try {
    const { cmd, args } = judgeBackend === "claude"
      ? { cmd: "claude", args: ["--setting-sources", "", "-p", prompt, "--model", CLAUDE_JUDGE_MODEL, "--output-format", "text", "--max-turns", "1", "--tools", "", "--no-session-persistence"] }
